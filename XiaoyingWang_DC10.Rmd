...
```
e. The case will not change, however, there are only two variable here which are name and count. Therefore, select() these two variables, name and count from the raw data to give the output table F.
```{r}
BabyNames%>%
  select(name,count)
```
